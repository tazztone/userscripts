...
- **Auto-Close Confirmation Screen**: After form submission, a polling loop runs every 200ms checking if the form has been removed from the dialog (which occurs when the success confirmation is displayed). Once the form is gone, the script clicks the close button (`.AbstractDialog_CloseButton`) to close the modal automatically. If a validation error occurs and the form remains, it times out after 3 seconds without closing, allowing the user to view the error message.
